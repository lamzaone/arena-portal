...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "vip-upsert") {
      ...
      if (!/^(ULTIMATE|DIAMOND|GOLD|SILVER|STANDARD)$/.test(group) || !validDuration(durationMinutes)) return redirect(request, "vips", "error", "vip-details");
      await upsertStaffVip({ steamId, name, group, durationMinutes, previousGroup });
    ...
    if (action === "vip-remove") {
      if (!actor.canManageVips) return redirect(request, "vips", "error", "vip-permission");
      const group = String(formData.get("group") ?? "").trim();
      if (!/^(ULTIMATE|DIAMOND|GOLD|SILVER|STANDARD)$/.test(group)) return redirect(request, "vips", "error", "vip-details");
      await removeStaffVip({ steamId, group });
