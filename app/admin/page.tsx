...
import { getStaffAdmins, getStaffAppeals, getStaffModeration, getStaffTickets, getStaffVips, type CaseMessage, type StaffAdmin, type StaffSanction } from "@/lib/data/portal-repository";
import { getSteamProfiles, type SteamProfile } from "@/lib/steam/profiles";

type StaffTab = "bans" | "admins" | "vips" | "appeals" | "tickets";
...
function getTab(value: string | undefined): StaffTab {
  return tabs.some((tab) => tab.id === value) ? value as StaffTab : "bans";
}
...
function CaseMessages({ messages, steamProfiles }: { messages: CaseMessage[]; steamProfiles: Map<string, SteamProfile> }) {
  ...
  return <div className="case-message-list">{messages.map((message) => {
    ...
    return <div key={message.id} className={`case-message ${message.authorType}`}><header className="case-message-header"><div><b>{authorName}</b><small>{message.authorType === "staff" ? "Staff reply" : "Player reply"}</small></div><time dateTime={message.createdAt}>{formatPortalDate(message.createdAt)}</time></header><p>{message.body}</p>{message.attachments.map((attachment) => <a key={attachment.id} href={`/api/case-attachments/${attachment.id}`} target="_blank" rel="noreferrer">View screenshot: {attachment.fileName}</a>)}</div>;
  })}</div>;
}
...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  const profileIds = [
    ...(moderation?.bans.map((ban) => ban.steamId) ?? []),
    ...(appealPage?.appeals.flatMap((appeal) => [appeal.steamId, ...appeal.messages.map((message) => message.authorId)]) ?? []),
  ...
  return <main className="tapped-page staff-page"><div className="shell"><SiteHeader authenticated />
    ...
    <nav className="staff-tabs" aria-label="Staff panel sections">{tabs.filter((item) => item.id !== "appeals" || access.canUnban).map((item) => <Link key={item.id} className={tab === item.id ? "active" : ""} href={staffLink(item.id)}>{item.label}</Link>)}</nav>
