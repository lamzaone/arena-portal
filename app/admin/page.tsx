...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  return <main className="tapped-page staff-page"><div className="shell"><SiteHeader authenticated />
    ...
    <nav className="staff-tabs" aria-label="Staff panel sections">{tabs.filter((item) => item.id !== "appeals" || access.canUnban).map((item) => <Link key={item.id} className={tab === item.id ? "active" : ""} href={staffLink(item.id)}>{item.label}</Link>)}{access.canViewEconomy ? <Link href="/admin/items">Items</Link> : null}{access.canViewEconomy ? <Link href="/admin/inventories">Inventories</Link> : null}{access.canManageEconomy ? <Link href="/admin/redeem">Redeem codes</Link> : null}</nav>
