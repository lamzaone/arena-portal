...
const pool = {
  async query(sql, values = []) {
    if (sql.includes("information_schema.COLUMNS")) return [[{ ready: effectStorageLength >= 1024 ? 3 : 0 }], []];
...
test("custom effect palettes survive create, read, update and reset", async () => {
  const { tagId } = await createIdentityChatTag({ ...base, key: "group.customfx", requestKey: "customfx-create",
    tagStyle: "glow cycle c=ff0000,0000ff f=0.25 i=3", nameStyle: "wave c=00ff00", messageStyle: "sparkle" });
  const read = async () => (await getIdentityAdminSnapshot()).tags.find((entry) => entry.id === tagId);
  assert.equal((await read()).tagStyle, "glow cycle c=FF0000,0000FF f=0.25 i=3");
  assert.equal((await read()).nameStyle, "wave c=00FF00");
  await updateIdentityChatTag({ ...base, tagId, requestKey: "customfx-edit", enabled: true, tagStyle: "gradient c=123456,ABCDEF" });
  assert.equal((await read()).tagStyle, "gradient c=123456,ABCDEF");
  await updateIdentityChatTag({ ...base, tagId, requestKey: "customfx-clear", enabled: true });
  assert.equal((await read()).tagStyle, "");
  db.prepare("DELETE FROM portal_identity_chat_tags WHERE id = ?").run(tagId);
});
...
test("independent effects survive saving, disabling, reloading and re-enabling", async () => {
  const style = "glow pulse cycle glow.c=112233 glow.i=2 glow.r=3 pulse.f=0.25 pulse.i=3 cycle.c=FF0000,00FF00,0000FF cycle.f=1 cycle.m=steps";
  assert.ok(style.length > 96);
  const { tagId } = await createIdentityChatTag({ ...base, key: "group.independent", requestKey: "independent-create", tagStyle: style });
  const read = async () => (await getIdentityAdminSnapshot()).tags.find((entry) => entry.id === tagId);
  try {
    assert.equal((await read()).tagStyle, style);
    const disabled = style.replace("glow pulse cycle", "pulse cycle");
    await updateIdentityChatTag({ ...base, tagId, requestKey: "independent-off", tagStyle: disabled, enabled: true });
    assert.equal((await read()).tagStyle, disabled);
    await updateIdentityChatTag({ ...base, tagId, requestKey: "independent-on", tagStyle: style, enabled: true });
    assert.equal((await read()).tagStyle, style);
  } finally { db.prepare("DELETE FROM portal_identity_chat_tags WHERE id = ?").run(tagId); }
});

test("96-character schema rejects long effects with migration guidance instead of truncating", async () => {
  const previous = (await getIdentityAdminSnapshot()).tags;
  effectStorageLength = 96;
  const style = "glow pulse cycle glow.c=112233 glow.i=2 glow.r=3 pulse.f=0.25 pulse.i=3 cycle.c=FF0000,00FF00,0000FF cycle.f=1 cycle.m=steps";
  try {
    const expected = (error) => error.code === "independent_chat_effects_migration_required" && error.message.includes("034_independent_chat_effects.sql");
    await assert.rejects(createIdentityChatTag({ ...base, key: "group.toolong", requestKey: "long-create", tagStyle: style }), expected);
    await assert.rejects(updateIdentityChatTag({ ...base, tagId: 1, requestKey: "long-update", tagStyle: style, enabled: true }), expected);
    assert.deepEqual((await getIdentityAdminSnapshot()).tags, previous);
  } finally { effectStorageLength = 1024; }
});
