...
New group roles start with zero Discord permissions; staff can configure permissions and subsequent syncs preserve them. All enabled AdminCore, VIP and custom groups are included. A separate managed TAPPED STAFF role follows effective AdminCore membership. Keep the bot above managed roles and enable Server Members Intent. Run one active bot process per guild. Startup registers /help, /link, /account, /sync and /sync-all; see the bot README for access checks. Notification settings on individual Discord accounts still control which alerts users receive.
...
4. Keep `SITE_URL`, `SESSION_SECRET`, `PORTAL_DATABASE_URL`, `GAME_DATABASE_URL`, `GAME_SERVER_GUID` and `GAME_VIP_SERVER_ID` configured normally. The snapshot ensures the portal's existing identity catalogue bootstrap, which can synchronize native group definitions into Arena authority and its portal projection. Membership resolution reads scoped Native Admins.Core/VIPCore and Arena authority; all required sources must succeed before role changes proceed.
5. The portal SQL account needs `SELECT`, `INSERT`, `UPDATE` on the new Discord tables and existing `portal_discord_links`, plus `INSERT` on `portal_audit_events`. Existing identity/case access remains necessary. Tables use InnoDB; notification claims use short `FOR UPDATE` transactions compatible with the current MariaDB 10.5 host. Discord delivery happens after commit, so network calls never hold claim locks. `SKIP LOCKED` is deliberately avoided because [MariaDB introduced it in 10.6](https://mariadb.com/docs/server/reference/sql-statements/data-manipulation/selecting-data/select).
